...
use crypto::threshold_sig::Signature;
use crypto_blstrs::blstrs::{G2Affine, G2Projective};
use crypto_blstrs::poly_commit::kzg::BlstrsKZG;
use crypto_blstrs::threshold_sig::{PartialBlstrsPublicKey, PartialBlstrsSignature};
use crypto_blstrs::vector_commit::BlstrsKZGVec;
use network::tokio::sync::oneshot;

use crate::{G1Projective, Group, Scalar};
...
pub struct SendMsg {
    ...
    pub committee_proof: Option<PartialBlstrsSignature>
}
...
impl SendMsg {
    pub fn new(c: G1Projective, r: Vec<G1Projective>, r_incl: G1Projective, s: Vec<G1Projective>, s_incls: Vec<G1Projective>, y_s: Vec<(Scalar, G1Projective)>,
               y_r: Vec<(G1Projective, G1Projective)>, committee_proof: Option<PartialBlstrsSignature>) -> Self {
        Self { c, r, r_incl, s, s_incls, y_s, y_r, committee_proof }
    }

    pub fn is_correct(&self, poly_commit: &BlstrsKZG, vec_commit: &BlstrsKZGVec, own_idx: usize, num_peers: usize, committee_prob: &Option<(usize, usize)>) -> bool {
        if let Some((num, denom)) = committee_prob {
            if self.committee_proof.is_none() {
                return false;
            }
            if !self.committee_proof.unwrap().rand_coin(*num, *denom).expect("Bad committee prob") {
                return false;
            }
        }
