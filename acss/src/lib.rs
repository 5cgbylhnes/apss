...
use crypto::threshold_sig::{CombinableSignature, PartialKey, SecretKey, Signature};
use crypto::VecCommit;
use crypto_blstrs::blstrs::{G1Compressed, G1Projective, Scalar};
...
use crypto_blstrs::threshold_sig::{BlstrsSignature, PartialBlstrsPublicKey};
...
impl HavenSenderParams {
    pub fn new(value: Scalar, generator: G1Projective, committee_prob: Option<(usize, usize)>) -> Self {
        assert!(committee_prob.is_none() || committee_prob.unwrap().1 != 0, "Zero division");
        Self { value, generator, committee_prob }
    }
}
...
impl HavenSender {
    pub async fn run(&mut self) {
        ...
        let HavenSenderParams{value, generator,  committee_prob } = self.additional_params.take().expect("No additional params given!");
        let committee_proof = match committee_prob {
            Some((num, denom)) => {
                let committee_proof = self.params.node.get_sk_share().sign(
                    bincode::serialize(&self.params.id).expect("ID serialize failed!"),
                    format!("{}-Haven-Sender-Committee", self.params.dst));
                if !committee_proof.rand_coin(num, denom).expect("Bad probabilities") {
                    self.params.handle.handle_stats_end().await;
                    return;
                }
                Some(committee_proof)
            }
            None => None
        };
        ...
        select! {
            Ok((c, rec_poly_commit, r_incl, share_poly_comms, s_incls, mut y_ss, y_r)) = rx_oneshot => {
                for (i, y_s) in y_ss.drain(0..).enumerate() {
                    let send_msg = SendMsg::new(c, rec_poly_commit.clone(), r_incl.clone(), share_poly_comms.clone(), s_incls.clone(), y_s, y_r.clone(), committee_proof.clone());
...
impl HavenReceiverParams {
    pub fn new(sender: usize, committee_prob: Option<(usize, usize)>) -> Self {
        Self { sender, committee_prob }
    }
}
...
impl HavenReceiver {
    pub async fn run(&mut self) {
        let HavenReceiverParams{ sender: acss_sender, committee_prob } = self.additional_params.take().expect("No additional params!");
...
mod tests {
    ...
    use protocol::{Node, run_protocol};
    ...
    async fn test_acss() {
        ...
        let params = HavenSenderParams::new(value.clone(), generator.clone(), prob);
        let _ = run_protocol!(HavenSender, handles[0].clone(), nodes[0].clone(), id.clone(), dst.clone(), params);

        let add_params = HavenReceiverParams::new(nodes[0].get_own_idx(), prob);
    ...
    //#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    /*
    async fn test_acss_committee() {
        let ports: Vec<u16> = vec![10103, 10104, 10105, 10106];
        let node_addrs = ports.iter().map(|p| SocketAddr::new(IpAddr::from_str("127.0.0.1").unwrap(), *p)).collect();
        let generators = (G1Projective::generator(), G2Projective::generator());
        let vec_params = BlstrsKZGVecParams::new(generators.0, generators.1, "DST".to_string());
        let nodes: Vec<_>= Node::<BlstrsSignature, BlstrsKZG, BlstrsKZGVec>::generate_nodes(node_addrs, 2, 3,generators, vec_params).unwrap().into_iter().map(|n| Arc::new(n)).collect();
        let managers: Vec<_> = nodes.iter().map(|n| n.spawn_manager(false)).collect();
        ...
        let prob: Option<(usize, usize)> = Some((1, 4));
        let params = HavenSenderParams::new(value.clone(), generator.clone(), prob);
        // node 3 is the only one in the committee that has expected size 1.
        let (sender_idx, sender) = nodes.iter().enumerate().filter(|(i, n)| n.peer_idx_map.get(&n.sk_share.to_pk()).unwrap() == &3).collect::<Vec<_>>()[0];
        let _ = run_protocol!(HavenSender, managers[sender_idx].clone(), sender.clone(), id.clone(), dst.clone(), params);

        let add_params = HavenReceiverParams::new(sender.sk_share.to_pk().clone(), prob);
        for i in 0..nodes.len() {
            let (tx, rx) =
                run_protocol!(HavenReceiver, managers[i].clone(), nodes[i].clone(), id.clone(), dst.clone(), add_params.clone());
            txs.push(tx);
            rxs.push(rx);
        }
        ...
        for (i, rx) in rxs.iter_mut().enumerate() {
            match rx.recv().await {
                Some(ACSSDeliver { y, feldman, .. }) => {
                    assert_eq!(crypto::eval(&feldman, &Scalar::zero(), G1Projective::identity()), generator.mul(&value));
                    points.push((Scalar::from(*nodes[0].peer_idx_map.get(&nodes[i].sk_share.to_pk()).unwrap() as u64 + 1), y));
                },
                None => assert!(false),
            }
        }
